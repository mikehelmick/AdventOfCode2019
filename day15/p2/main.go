...
func bfs(room map[pos]int64, start pos) int64 {
	q := make([]pos, 0, 100)
	q = append(q, start)
	dist := int64(0)
...
func main() {
	...
	for !done {
		...
		if x.Done {
			...
			steps++
			// let's assume we can fully discover the grid in 1M steps of random walk
			if steps >= 5000000 {
				inC <- 0
				continue
			}
	...
	rounds := bfs(room, tankPos)
	printScreen(room, tankPos, d)
	fmt.Printf("Filled cells in %v rounds\n", rounds)
