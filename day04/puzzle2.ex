defmodule Puzzle do
  ...
  def rule3([a,a,b,_,_,_]) when a != b, do: true
  def rule3([a,b,b,c,_,_]) when a != b and b != c, do: true
  def rule3([_,a,b,b,c,_]) when a != b and b != c, do: true
  def rule3([_,_,a,b,b,c]) when a != b and b != c, do: true
  def rule3([_,_,_,a,b,b]) when a != b, do: true
  def rule3(_), do: false

  def solve(x, y, acc) when x > y, do: acc
  def solve(x, y, acc) do
    digits = to_list(x)

    case {rule1(digits), rule2(digits), rule3(digits)} do
      {true, true, true} -> solve(x+1, y, acc ++ [x])
      _ -> solve(x+1, y, acc)
    end
  end
end
