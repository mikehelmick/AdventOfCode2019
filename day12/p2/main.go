...
func main() {
	...
	// when each axis return to initial state.
	x, y, z := int64(0), int64(0), int64(0)

	printAll(moons, 0)
	for s := int64(1); x == 0 || y == 0 || z == 0; s++ {
		...
		for i := 0; i < 3; i++ {
			if axisEquals(iState, moons, i) {
				if i == 0 && x == 0 {
					x = s
					log.Printf("x period %v", s)
				} else if i == 1 && y == 0 {
					log.Printf("y period %v", s)
					y = s
				} else if i == 2 && z == 0 {
					log.Printf("z period %v", s)
					z = s
				}
			}
		}
	...
	// answer is LCM of period for each axis
	res := lcm(x, y, z)
	log.Printf("Return to state after: %v", res)
}
